...
@Tag("fast")
class WeakConnectivityCheckerTest {
  private static Stream<Arguments> getWeaklyConnectedTestArguments() {
    ...
    return Stream.of(
        Arguments.of(weak1, true),
        Arguments.of(weak2, true),
        Arguments.of(weak3, true),
  ...
  @ParameterizedTest
  @MethodSource("getWeaklyConnectedTestArguments")
  void whenIsWeaklyConnectedIsCalled_thenTheCorrectAnswerIsReturned(
      final Graph<Integer> graph, final boolean expectedResult) {

    assertThat(WeakConnectivityChecker.<Integer>of().isWeaklyConnected(graph))
